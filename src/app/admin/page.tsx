...
export default function AdminPage() {
  ...
  useEffect(() => {
    const auth = sessionStorage.getItem("taxi_02_admin_auth");
    if (auth === "true") {
  ...
  const loadData = () => {
    try {
      const storedEvents = JSON.parse(
        localStorage.getItem("taxi_02_events") || "[]"
      );
      const storedReservations = JSON.parse(
        localStorage.getItem("taxi_02_reservations") || "[]"
      );
      const storedContacts = JSON.parse(
        localStorage.getItem("taxi_02_contacts") || "[]"
      );
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (password === ADMIN_PASSWORD) {
      setIsAuthenticated(true);
      sessionStorage.setItem("taxi_02_admin_auth", "true");
      setError("");
    } else {
  ...
  const handleLogout = () => {
    ...
    sessionStorage.removeItem("taxi_02_admin_auth");
    setPassword("");
  };
      ...
      <div className="min-h-screen bg-surface flex items-center justify-center px-4">
        <div className="w-full max-w-md">
          <div className="card-premium text-center">
            <div className="w-16 h-16 rounded-xl bg-gold-400/8 border border-gold-400/20 flex items-center justify-center mx-auto mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="card-premium">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-2 border-b border-gold-400/15">
                <span className="text-gray-600">Événements (24h)</span>
                <span className="text-foreground font-semibold">{recentEvents}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-gold-400/15">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              {events
                ...
                .map((event, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between py-2 px-3 rounded-lg bg-surface-100/50 hover:bg-surface-100 transition-colors"
                  >
                    <div className="flex items-center gap-3">
...
function StatCard({
  ...
  return (
    <div className="card-premium text-center">
      <div className="w-10 h-10 rounded-lg bg-gold-400/8 flex items-center justify-center mx-auto mb-3">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className={highlight ? "text-gold-400" : "text-gray-700"}>
          {label}
        </span>
...
function EventBadge({ event }: { event: string }) {
  ...
  const color = colors[event] || "bg-gray-500/20 text-gray-600";
