...
function storeEventLocally(eventName: AnalyticsEvent, params?: EventParams) {
  ...
  try {
    const events = JSON.parse(localStorage.getItem("taxi_02_events") || "[]");
    events.push({
      event: eventName,
      params,
    ...
    localStorage.setItem("taxi_02_events", JSON.stringify(events));
    ...
    if (eventName === "reservation_completed") {
      ...
      localStorage.setItem("taxi_02_reservations", JSON.stringify(reservations));
    }
    ...
    if (eventName === "contact_form_submitted") {
      ...
      localStorage.setItem("taxi_02_contacts", JSON.stringify(contacts));
    }
  } catch {
