...
const services = [
  {
    ...
    badge: "Conventionné",
    image: "/images/vehicule/vehicule-urgences.png",
    imageAlt: "Taxi 02 Strasbourg — véhicule devant les urgences, transport médical CPAM",
  },
  {
    icon: Plane,
  ...
  },
  {
    ...
    image: "/images/photos/enseigne-taxi.jpg",
    imageAlt: "Enseigne Taxi 02 Strasbourg — Service professionnel entreprise",
  },
  {
    icon: Baby,
          ...
          {services.map((service, index) => (
            ...
            >
              <Link href={service.href} className="block h-full">
                <div className="group relative overflow-hidden rounded-xl border border-gold-400/20 hover:border-gold-400/40 transition-all duration-300 h-full bg-surface-50">
                  {/* Image */}
                  {service.image ? (
                    <div className="relative h-48 sm:h-44 overflow-hidden">
                      ...
                      <div className="absolute bottom-3 left-4 w-12 h-12 rounded-xl bg-white/90 border border-gold-400/30 flex items-center justify-center backdrop-blur-sm shadow-lg">
                        <service.icon className="w-6 h-6 text-gold-400" />
                      </div>
                    </div>
                  ) : (
                    <div className="relative h-44 bg-gradient-to-br from-gold-50 to-surface-50 flex items-center justify-center">
                      {/* Icon centered */}
                      <div className="w-16 h-16 rounded-xl bg-gold-50 border border-gold-400/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  ...
                  <div className="p-4 sm:p-5">
                    {service.badge && (
                      <span className="inline-block text-xs bg-gold-50 text-gold-400 font-semibold px-2.5 py-0.5 rounded-full border border-gold-400/20 mb-2">
                        {service.badge}
                      </span>
                    )}
                    <h3 className="text-base sm:text-lg font-semibold text-foreground mb-2 group-hover:text-gold-400 transition-colors leading-tight">
                      {service.title}
                    </h3>
